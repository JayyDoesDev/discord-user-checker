async function submit() {
  ...
  if (input) {
    ...
    document.getElementById("user-username").innerHTML =
      "Username: " + username;
    document.getElementById("user-discrim").innerHTML =
    ...
    status
      ? (document.getElementById("status").src = `../assets/${status}.png`)
    ...
    if (res.presence.activities) {
      ...
      document.getElementById("presence").innerHTML =
        "Presence: " + typeString + " " + activity;
    } else {
      document.getElementById("presence").innerHTML =
        "Presence: not playing anything";
      return;
    }
  } else {
